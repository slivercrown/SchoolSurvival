...
var JOYSTICK = preload("res://Joystick.tscn").instance()
var joystick_moving
var joystick_aim

func _ready():
	add_child(JOYSTICK)
	joystick_moving = JOYSTICK.get_node("Moving/TouchScreenButton")
	joystick_aim = JOYSTICK.get_node("Aim/TouchScreenButton")
...
const INVENTORY_SCENE = preload("res://Inventory/Inventory.tscn")
const ITEMBUTTON_SCENE = preload("res://UI/ItemButton/ItemButton.tscn")
...
func _make_itembutton():
	if oldbutton ==null:
		oldbutton = ITEMBUTTON_SCENE.instance()
		add_child(oldbutton)
	oldbutton.start_at(((grid_position.x*2)+1) * 240 + 144 , ((grid_position.y*2)+1) * 135 - 109)
...
func _physics_process(delta):
	if is_network_master():
		...
		var output = joystick_aim.get_value()
...
func shoot():
	if is_network_master():
		#if pow(get_node("../JoystickAim/Joystick").output[0], 2) + pow(get_node("../JoystickAim/Joystick").output[1], 2) >= 0.7:
			mouse_position = Vector2(joystick_aim.get_value()[0]*10000,joystick_aim.get_value()[1]*10000)
			$Sprite_Gun.look_at(mouse_position) # 무기 방향
			rset_unreliable('Smouse_position', mouse_position)
	else:
		$Sprite_Gun.look_at(mouse_position) # 무기 방향
...
func get_input_axis():
	...
	axis = joystick_moving.get_value()
	...
	if(axis[0] < 0): 
			#if pow(axis[0], 2) + pow(axis[1], 2) < 0.7:
			#if pow(get_node("../JoystickAim/Joystick").output[0], 2) + pow(get_node("../JoystickAim/Joystick").output[1], 2) < 0.7:
				$Sprite_Player.set_flip_h(true)
				sprite_flip = true
	elif(axis[0] > 0):
...
func item_loop(equip):
	if using:
		if actitem != ACTITEM.NULL && can_use:
			...
			if actitem == ACTITEM.TACK && equip == 1:
				...
				if sprite_flip:
					...
					g.start_at($Sprite_Gun.get_global_rotation(),Vector2(global_position.x+32,global_position.y-4))  
			elif actitem == ACTITEM.GUM && equip == 2:
...
func _on_Hitbox_area_entered(area):
	...
	if actitem == ACTITEM.NULL && area.get("TYPE") != null && area.get("TYPE") == "ACTITEM" && area.collectible:
					...
					if area.get("NAME") == 2 :
						oldinven.add_item(2)
						oldbutton._change_image(2)
						#World.imageup(1)
					
					elif area.get("NAME") == 1 :
						oldinven.add_item(1)
						oldbutton._change_image(1)
						#World.imageup(1)
				...
				if area.get("NAME") == 2 :
						oldinven.add_item(2)
						oldinven.minus_item(1)
						oldbutton._change_image(2)
					
				elif area.get("NAME") == 1 :
						oldinven.add_item(1)
						oldinven.minus_item(2)
						oldbutton._change_image(1)
